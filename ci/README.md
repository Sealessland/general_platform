...
- `benchmark.yml` 手动运行只写 Job Summary 并上传 artifact；只有 `main` push 会回写 README 性能表，避免功能分支污染基线
- `release.yml` 在 `vX.Y.Z` tag 或手动触发时发布三个 GHCR 镜像；当前没有绑定生产环境，发布镜像就是 CD 边界

完整操作教程见 `docs/tutorials/github-actions-ci-cd.md`。
