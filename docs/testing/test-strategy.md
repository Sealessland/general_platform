...
- 集成测试：PostgreSQL 迁移、JWT/Redis 共享认证、Redis catalog 读侧适配器、事件链路
...
- JWT/Redis 认证测试：验证标准 claims、HS256 算法白名单、Access/Refresh 类型隔离、一次性轮换和跨实例撤销
- Redis 适配器测试：验证商品/SKU 缓存，以及 Lua 令牌桶在并发请求下不会突破 burst
...
- `BenchmarkPostgresKafkaOutboxRelay` 使用真实 PostgreSQL outbox 和真实 Kafka producer，衡量 outbox relay 路径。
...
- Refresh Token 重放、跨实例登出不一致、两个实例并发执行同一数据库迁移
- 退款金额超过支付金额
- 优惠重复使用
- 高流量 SQL 缺索引
...
- `backend/internal/redcart/infrastructure/auth/jwt_manager_test.go` 覆盖标准 JWT、算法限制、一次性 Refresh 轮换和两个 Manager 共享 Redis 的跨实例撤销。
- `backend/internal/redcart/infrastructure/redis/catalog_cache_repository_test.go` 与 `rate_limiter_test.go` 覆盖商品/SKU 热缓存、跨实例读取、写后失效和 Lua 原子限流。
- `backend/internal/redcart/interfaces/httpapi/server_postgres_test.go`、`server_benchmark_test.go`、`live_benchmark_test.go` 和 `server_test_helpers_test.go` 覆盖 PostgreSQL-backed HTTP 集成路径、PostgreSQL-backed handler benchmark 和 live HTTP benchmark。
