...
- `backend/internal/redcart/infrastructure/redis/session_repository_test.go` 与 `catalog_cache_repository_test.go` 在真实 Redis + PostgreSQL 下覆盖 Redis session token 存取、TTL 解析、商品/SKU 热缓存、跨实例 Redis 读取和写后失效行为。
